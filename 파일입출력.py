...
# 사용자가 터미널에 대해 입/출력이 아니고 파일에 대한 입출력 하기 위해서는 파일에 대한 입/출력에 대한 별도의 함수를 사용 합니다.
...
# with 키워드 : 프로그램이 길어지면 open()과 close()사이에 많은 코드가 들어가 close를 호출하지 않는 문제가 발생 할 수 있음
...
#         password = my_str[:3] + str(len(my_str)) + str(my_str.count("o")) + date + "!" + "kdk"
#         fd.write(password + "\n")
